...
import { createExportHTML, downloadHTML } from "@/lib/export-html";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function HeaderActions({ user, projectId }: HeaderActionsProps) {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      // 查找入口点
      const entryPoint = files.has('/App.jsx') ? '/App.jsx' :
                        files.has('/App.tsx') ? '/App.tsx' :
                        files.has('/index.jsx') ? '/index.jsx' :
                        files.has('/index.tsx') ? '/index.tsx' : '/App.jsx';

      const html = createExportHTML(files, entryPoint);
